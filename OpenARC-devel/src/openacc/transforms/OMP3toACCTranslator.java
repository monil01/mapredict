...
public class OMP3toACCTranslator extends TransformPass {
  ...
  @Override
    public void start() 
    {
      Tools.exit("Conversion from OpenMP 3.0 to OpenACC not currently supported.");
          ...
          if ((at instanceof AnnotationStatement) || (at instanceof AnnotationDeclaration) )
          {
            ...
            if( old_annot == null ) {
              if( !(pAnnot instanceof OmpAnnotation) ) {
                PrintTools.println("\n[WARNING in OMP3toACCTranslator] Pragma annotation, "
          ...
          for (int i = 0; i < pragmas.size(); i++) 
          {
            ...
            if(pragma.containsKey("target"))
            {
              if( !(at instanceof ForLoop) ) {
                ...
                if (directive != null) {
                  Tools.exit("[ERROR in OMP3toACCTranslator.start()] An OpenMP "+ directive + " directive is allowed only to for-loops, "
                ...
                while (flatIterator.hasNext())
                {
                  ...
                  if(stmtAnnot != null) {
                    ...
                    if( (ompList != null) && (ompList.size() > 0) ) {
                      Tools.exit("[ERROR in OMP3toACCTranslator] target construct allows only teams, parallel, and simd constructs in its region, but "
            ...
            if( pragmas != null ) {
              for (int i = 0; i < pragmas.size(); i++) 
              {
                ...
                if( !(at instanceof ForLoop) ) {
                  ...
                  if (directive != null) {
                    Tools.exit("[ERROR in OMP3toACCTranslator.start()] An OpenMP "+ directive + " directive is allowed only for for-loops, "
                        + "but the following construct illegally uses the " + directive + " directive:\n"
                        + "OpenMP construct: " + at + "\n" + AnalysisTools.getEnclosingContext(at));
                  }
                }
      ...
      for( FunctionCall fCall : funcCallList ) {
        ...
        if( fName.equals("omp_target_alloc") ) {
          ...
          if( tt instanceof Statement ) {
            ...
            Tools.exit("[ERROR in OMP3toACCTranslator.start()] fail to convert omp_target_alloc() funcion; exit!");
          }
          ...
          if( tt instanceof Statement ) {
            ...
            Tools.exit("[ERROR in OMP3toACCTranslator.start()] fail to convert omp_target_free() funcion; exit!");
          }
          ...
          PrintTools.println("[WARNING in OMP3toACCTranslator.start()] converting omp_target_is_present() function is incomplete; "
          ...
          if( tt instanceof Statement ) {
            ...
            Tools.exit("[ERROR in OMP3toACCTranslator.start()] fail to convert omp_target_is_present() funcion; exit!");
          }
          ...
          if( tt instanceof Statement ) {
            ...
            Tools.exit("[ERROR in OMP3toACCTranslator.start()] fail to convert omp_target_associate_ptr() funcion; exit!");
          }
          ...
          if( tt instanceof Statement ) {
            ...
            Tools.exit("[ERROR in OMP3toACCTranslator.start()] fail to convert omp_target_disassociate_ptr() funcion; exit!");
          }
          ...
          //??
          Tools.exit("[ERROR in OMP3toACCTranslator.start()] fail to convert omp_target_memcpy() funcion; exit!");
        } else if( fName.equals("omp_target_memcpy_rect") ) {
          Tools.exit("[ERROR in OMP3toACCTranslator.start()] fail to convert omp_target_memcpy_rect() funcion; exit!");
  ...
  private void parse_parallel_pragma(OmpAnnotation pragma)
  {
    ...
    if( (ompTargetAnnot != null) && (accAnnot == null) ) {
      ...
      if(num_threads != null) {
        if( accAnnot.containsKey("num_workers") ) {
          Expression num_workers = accAnnot.get("num_workers");
          if( (num_workers instanceof IntegerLiteral) && (num_threads instanceof IntegerLiteral) ) {
            ...
            if( t2 < t1 ) {
              accAnnot.put("num_workers", num_threads);
            }
          } else {
            //[DEBUG] conservatively use thread_limit value.
          }
        } else {
          accAnnot.put("num_workers", condition);
        }
      }
  ...
  private void parse_target_pragma(OmpAnnotation pragma, OmpRegion regionType)
  {
    ...
    if( regionType == OmpRegion.Target ) {
      ...
      if( taskMapping.equals("coarse_grained") ) {
        ...
        Tools.exit("[ERROR in OMP3toACCTranslator.start()] Unexpected argument (" + taskMapping + ") for the map clause in the following OpenARC devicetask construct; exit!\n"
  ...
  private CompoundStatement handleDependClause ( OmpAnnotation pragma, int numDependVariables, Set<SubArray> inSet,
    ...
    for( SubArray subA : inSet ) {
      ...
      if( !foundDimensions ) {
        Tools.exit("[ERROR in OMP3toACCTranslator.parse_target_pragma()] Dimension information " +
