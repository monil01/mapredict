...
public class ACCWorkshareLoopNormalization extends TransformPass {
	...
	public void start() {
		...
		//[DEBUG on July 8, 2020] add loop clause to work on general OpenACC loops.
		clauseSet.add("loop");
		//clauseSet.add("tile");
						...
						if( cStmt.containsDeclaration(newIndexSymDecl) ) {
							...
							if( pp instanceof ForLoop ) {
								...
								if( (pp != null) && (pp instanceof CompoundStatement) ) {
									...
									Declaration existingSymDecl = SymbolTools.findSymbol(ppStmt, newIndexSym.getSymbolName());
									if( existingSymDecl == null ) {
										ppStmt.addDeclaration(newIndexSymDecl);
										IRTools.replaceAll(fLoop, newIndexVar, newIndexVar);
									} else {
										Expression existingIndexVar = existingSymDecl.getDeclaredIDs().get(0);
										Symbol existingIndexSymbol = SymbolTools.getSymbolOfName(existingIndexVar.toString(), ppStmt);
										if( existingIndexSymbol == null) {
											Tools.exit("==> [ERROR in ACCWorksharingLoopNormalization] Cannot find an index symbol, "+
										existingIndexSymbol.getSymbolName() + "; exit!");
										}
										IRTools.replaceAll(fLoop, newIndexVar, new Identifier(existingIndexSymbol));
									}
								}
							}
						}
