...
public class SlidingWindowTransformation extends TransformPass {
	...
	protected List<Statement> tempComputeRegions = null;

	/**
	 * @param program
	 */
	public SlidingWindowTransformation(Program program, int OPENARC_ARCH, List<Statement> tempCompRegions) {
		super(program);
		targetArch = OPENARC_ARCH;
		tempComputeRegions = tempCompRegions;
	}
	...
	public void start() {
		...
		List<ACCAnnotation>  computeAnnots = AnalysisTools.collectPragmas(program, ACCAnnotation.class, ACCAnnotation.computeRegions, false);
		for( ACCAnnotation compAnnot : computeAnnots ) {
			Annotatable compAt = compAnnot.getAnnotatable();
			List<ARCAnnotation>  windowAnnots = IRTools.collectPragmas(compAt, ARCAnnotation.class, "window");
			if( windowAnnots.isEmpty() ) {
				continue;
			} else {
				if( targetArch == 6 ) {
					Statement tRegion = (Statement)compAt;
					Statement clonedRegion = tRegion.clone();
					List<ARCAnnotation>  clonedWindowAnnots = IRTools.collectPragmas(clonedRegion, ARCAnnotation.class, "window");
					for( ARCAnnotation tWAnnot : clonedWindowAnnots ) {
						Annotatable tWAt = tWAnnot.getAnnotatable();
						List<ARCAnnotation> arcAnnots = tWAt.getAnnotations(ARCAnnotation.class);
						tWAt.removeAnnotations(ARCAnnotation.class);
						for( ARCAnnotation tArcAnnot : arcAnnots ) {
							if( !tArcAnnot.containsKey("window") ) {
								tWAt.annotate(tArcAnnot);
							}
						}
					}
					CompoundStatement cStmt = (CompoundStatement)tRegion.getParent();
					TransformTools.addStatementAfter(cStmt, tRegion, clonedRegion);
					tempComputeRegions.add(tRegion);
				}
			}
				...
				for( ArrayAccess aAccess : arrayAccessList ) {
					if( aAccess.getArrayName().equals(input_array) ) {
						...
						for( Expression wExp : window_indices) {
							...
							Expression nExpSimReordered = TransformTools.applyAssociaiveProperty(nExpSim);
							Expression iterExpSimReordered = TransformTools.applyAssociaiveProperty(iterExpSim);
							...
							} else if (IRTools.containsExpression(nExpSimReordered, iterExpSimReordered)) {
								IRTools.replaceAll(nExpSimReordered, iterExpSimReordered, new IntegerLiteral(0)); 
								nExp = nExpSimReordered;
							} else if (IRTools.containsExpression(nExpSim, iterExpSim)) {
								IRTools.replaceAll(nExpSim, iterExpSim, new IntegerLiteral(0)); 
								nExp = nExpSim;
							} else {
							...
							if( aAccess.getArrayName().equals(input_array) ) {
								...
								for( Expression oExp : oldindices ) {
									...
									if (nExp.equals(iterExp)) {
										...
										IRTools.replaceAll(nExpSimReordered, iterExpSimReordered, target.clone());
										nExp = nExpSimReordered;
									} else if (IRTools.containsExpression(nExpSim, iterExpSim)) {
										//System.out.println("Simplify MATCH");
										...
										System.out.println("[WARNING in SlidingWindowTransformation] All input array index expressions " + 
												"must be of the form:\n  required: " + iterExp.toString() + " (" + iterExpSim + ")" +  
												" + CONST\n  found: " + nExp.toString() + " (" + nExpSim + ")");
