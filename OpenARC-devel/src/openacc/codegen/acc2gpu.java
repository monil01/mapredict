				...
				while(tokenizer.hasMoreTokens()) {
					...
					if( eqIndex != -1) {
						...
						if(opt.equals("enableAdvancedMapping") || opt.equals("vectorize") || opt.equals("devicetype") || opt.equals("vectorfriendlyanalysis")) {
							try {
								int arg = new Integer(option.substring(eqIndex+1).trim()).intValue();
...
/*		if(applyInlining) {
		...
		if(convertOpenACCtoOpenMP4 || convertOpenMPtoOpenMP) {
			...
			ACCtoOMP4Translator acc2OMPTranslator = new ACCtoOMP4Translator(program, defaultNumAsyncQueues, enableAdvancedMapping, vectorizationMode);
			TransformPass.run(acc2OMPTranslator);
			cleanAnnotations();
			return;
		}
        ...
        if( (OPENARC_ARCH <= 0) || (OPENARC_ARCH == 5) )
        {
            ...
            ACC2CUDATranslator a2cpass = new ACC2CUDATranslator(program, refComputeRegionAnnots, OPENARC_ARCH);
            a2cpass.start();
        ...
        else
        {
        	if( OPENARC_ARCH == 6 ) {
        		PrintTools.println("[ACC2CUDATranslator] begin", 0);
        		ACC2CUDATranslator a2cupass = new ACC2CUDATranslator(program, refComputeRegionAnnots, 0);
        		...
        		ACC2OPENCLTranslator a2clpass = new ACC2OPENCLTranslator(program, refComputeRegionAnnots, 1);
        		...
        		PrintTools.println("[ACC2OPENCLTranslator] begin", 0);
        		a2clpass = new ACC2OPENCLTranslator(program, refComputeRegionAnnots, 3);
        		a2clpass.attachTempComputeRegions(tempComputeRegions, tempCompRegionLocations);
        		a2clpass.generateDeviceKernels();
        		PrintTools.println("[ACC2OPENCLTranslator] end", 0);
        		PrintTools.println("[OpenCLArrayFlattener] begin", 0);
        		arrayFlattener = new OpenCLArrayFlattener(program, assumeNoAliasingAmongKernelArgs, OPENARC_ARCH, 3);
        		arrayFlattener.start();
        		PrintTools.println("[OpenCLArrayFlattener] end", 0);
        		PrintTools.println("[OpenCLTranslationTools] begin", 0);
        		...
        		PrintTools.println("[ACC2OPENCLTranslator] begin", 0);
        		ACC2OPENCLTranslator a2clpass = new ACC2OPENCLTranslator(program, refComputeRegionAnnots, OPENARC_ARCH);
        		a2clpass.start();
        		PrintTools.println("[ACC2OPENCLTranslator] end", 0);
        		PrintTools.println("[OpenCLArrayFlattener] begin", 0);
