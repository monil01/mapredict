...
public class CompRegionConfAnalysis extends AnalysisPass {
	...
	private int maxNumWorkers = 1024; //Maximum number of threads per block in CUDA
	private int OPENARC_ARCH = 0;
	private int enableAdvancedMapping = 0;

	public CompRegionConfAnalysis(Program program, int target, int enableAdvdMapping) {
		super(program);
		OPENARC_ARCH = target;
		enableAdvancedMapping = enableAdvdMapping;
	}
	...
	private void calParallelRegionConfConfiguration( List<ACCAnnotation> parallelRegions ) {
		...
		for( ACCAnnotation cAnnot : parallelRegions ) {
			...
			if( cAnnot.containsKey("num_workers") ) {
				...
				List<ACCAnnotation> workerAnnots = AnalysisTools.ipCollectPragmas(at, ACCAnnotation.class, searchKeys, false, null);
			...
			searchKeys.add("independent");
			searchKeys.add("gang");
			List<ACCAnnotation> gangAnnots = AnalysisTools.ipCollectPragmas(at, ACCAnnotation.class, searchKeys, false, null);
			if( (gangAnnots == null) || (gangAnnots.isEmpty()) ) {
				...
				for( ACCAnnotation gAnnot : gangAnnots ) {
					ForLoop gLoop = (ForLoop)gAnnot.getAnnotatable();
					//[DEBUG] Added by Seyong on May 26, 2020
				...
				if( !cAnnot.containsKey("num_gangs") ) {
					//PrintTools.println("[DEBUG] Current compute region:\n" + at, 0);
					...
					for( ForLoop oGLoop : outermostGangLoops ) {
						...
						} else if( tnum_gangs != null ) {
							num_gangs = Symbolic.simplify( 
									new MinMaxExpression(false, num_gangs, tnum_gangs));
						} else if( iterspace == null ) {
							PrintTools.println("[WARNING in CompRegionConfAnalysis.calParallelRegionConfiguration()] the following loop does not have internal iterspace value!" + 
						"\nCurrent Loop: " + oGLoop + 
						AnalysisTools.getEnclosingContext(oGLoop), 0);
