...
/*        if( optionLevel == 1 ) {
        ...
        if( cRegionAnnots != null ) {
        	for( ACCAnnotation cAnnot : cRegionAnnots ) {
        		...
        		//[DEBUG] disable below so that OpenACC independent clause is added to all parallelizable loops in a compute region.
/*        		List<ACCAnnotation> loopAnnots = AnalysisTools.ipCollectPragmas(at, ACCAnnotation.class, "loop", null); 
        		...
        		if( cetusAnnots != null ) {
        			for( CetusAnnotation cetusAnnot : cetusAnnots ) {
        				...
        				ACCAnnotation aAnnot =  cat.getAnnotation(ACCAnnotation.class, "loop");
        				if( aAnnot == null ) {
        					if( optionLevel > 1 ) {
        						aAnnot = new ACCAnnotation("loop", "_directive");
        						cat.annotate(aAnnot);
        					} else {
        						continue;
        					}
        				}
        				if( !aAnnot.containsKey("seq") ) {
