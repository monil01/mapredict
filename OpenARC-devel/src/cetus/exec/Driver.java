            ...
            if (eq == -1) { // if value is not set
                ...
                if (options.contains(option_name)) {
                    if (option_name.equals("preprocessor")) {
                        ...
                        preprocessor = false;
                        //[DEBUG on June 17, 2020]
                        if( opt.substring(eq+1).equals("") ) {
                        	System.err.println("[ERROR in Driver.parseCommandLine()] option value is missing in the commandline option, " + opt + " ; exit!");
                        	Tools.exit(1);
                        }
                        // use the value from the command line
