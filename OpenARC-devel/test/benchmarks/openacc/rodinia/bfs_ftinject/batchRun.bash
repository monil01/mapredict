...
if [ $# -ge 1 ]; then
	if [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
		...
		echo "      $ batchRun.bash [TARGET] [FTVAR] [ITR] [RR] [RMODE] [FTKIND] [NUM_OF_NODES] [EDGELIST_SIZE] [INPUTFILE]"
		...
		echo "      and NUM_OF_NODES and EDGELIST_SIZE are the input sizes for the input file, INPUTFILE "
		echo "      (default: NUM_OF_NODES = 4096 and EDGELIST_SIZE = 24576 for INPUTFILE = graph4096.txt),"
		echo ""
		exit
	fi
fi
...
if [ $# -eq 9 ]; then
	...
	inputData1=4096
	inputData2=24576
	inputFile="${rodinia}/data/bfs/graph4096.txt"
elif [ $# -eq 5 ]; then
	target=$1
	ftvar=$2
	numIterations=$3
...
echo "RMODE: $RMODE"
echo "NUM_OF_NODES: $inputData1"
echo "EDGELIST_SIZE: $inputData2"
echo "INPUTFILE: $inputFile"

benchmark="bfs"

#Reference output data to be compared
#refOutput="bfsref.out"
...
FTVAR0Set=( "h_graph_visited" "h_graph_mask" "h_updating_graph_mask" "h_graph_nodes_starting" "h_graph_nodes_no_of_edges" "h_graph_edges" "h_cost" )
FTVAR1Set=( "h_graph_visited" "h_graph_mask" "h_updating_graph_mask" )
		...
		do	
			...
			if [ -f "${binaryDir}/${targetdir}/${binary_target}" ]; then
				...
				do
					...
					exeCmd="./${binary_target} 1 ${inputFile}"
					if [ $printprogress -gt 0 ]; then
