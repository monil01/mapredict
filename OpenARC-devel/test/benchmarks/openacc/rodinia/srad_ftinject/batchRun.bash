...
if [ $# -ge 1 ]; then
	if [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
		...
		echo "      $ batchRun.bash [TARGET] [FTVAR] [ITR] [RR] [RMODE] [FTKIND] [ROWS]"
		...
		echo "      and ROWS is the input size (default: ROWS = 1024), "
		echo ""
		exit
	fi
fi
...
echo "RMODE: $RMODE"
echo "ROWS: $inputData1"

benchmark="srad"

#Reference output data to be compared
refOutput="sradOutput_${inputData1}.txt"
#Output data to be generated.
tOutput="sradOutput.txt"
...
FTVAR0Set=( "J" "c" "dE" "dN" "dS" "dW" )
FTVAR1Set=( "J" "c" "dE" "dN" "dS" "dW" )
...
if [ ! -f ${binaryDirBase}/${refOutput} ]; then
	make CPU _ROWS_=${inputData1} _COLS_=${inputData1}
	cd ${binaryDirBase}
	./srad_CPU ${inputData1} ${inputData1} 0 127 0 127 1 0.5 100
	mv sradOutput.txt ${refOutput}
fi
		...
		do	
			...
			if [ -f "${binaryDir}/${targetdir}/${binary_target}" ]; then
				...
				do
					...
					exeCmd="./${binary_target} ${inputData1} ${inputData1} 0 127 0 127 1 0.5 100"
					if [ $printprogress -gt 0 ]; then
