...
if [ $# -ge 1 ]; then
	if [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
		...
		echo "      $ batchRun.bash [TARGET] [FTVAR] [ITR] [RR] [RMODE] [FTKIND] [_MAX_ROWS_]"
		...
		echo "      and _MAX_ROWS_ is the input size"
		echo "      (default: _MAX_ROWS_ = 2049),"
		echo ""
		exit
	fi
fi
...
#	make CPU _MAX_ROWS_=${inputData1}
#	cd ${binaryDirBase}
#	./nw_CPU ${max_rows} 10 1 
#fi
		...
		do	
			...
			if [ -f "${binaryDir}/${targetdir}/${binary_target}" ]; then
				...
				do
					...
					exeCmd="./${binary_target} ${max_rows} 10 1"
					if [ $printprogress -gt 0 ]; then
