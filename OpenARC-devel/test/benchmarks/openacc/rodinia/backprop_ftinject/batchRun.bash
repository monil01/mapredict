...
if [ $# -ge 1 ]; then
	if [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
		...
		echo "      $ batchRun.bash [TARGET] [FTVAR] [ITR] [RR] [RMODE] [FTKIND] [INPUTDATA]"
		...
		echo "      and INPUTDATA is the input size. (default: 65536),"
		echo ""
		exit
	fi
fi
...
echo "RMODE: $RMODE"
echo "Input Size: $inputData1"
inputData1=$((inputData1+1))

benchmark="backprop"

#Reference output data to be compared
#refOutput="backpropref.out"
#Output data to be generated.
#tOutput="backprop.out"
...
targetRegion="${RES_REGION0}${RES_REGION1}${RES_REGION2}${RES_REGION3}"
miscMacros="RES_REGION0=${RES_REGION0},RES_REGION1=${RES_REGION1},RES_REGION2=${RES_REGION2},RES_REGION3=${RES_REGION3}"

FTVAR0Set=( "input_units" "input_weights" )
FTVAR1Set=( "hidden_units" "hidden_weights" )
FTVAR2Set=( "hidden_units" "hidden_weights" "hidden_prev_weights" "output_delta" )
FTVAR3Set=( "input_units" "input_weights" "input_prev_weights" "hidden_delta" )
		...
		do	
			...
			if [ -f "${binaryDir}/${targetdir}/${binary_target}" ]; then
				...
				do
					...
					exeCmd="./${binary_target} ${inputData1}"
					if [ $printprogress -gt 0 ]; then
