...
if [ $# -ge 1 ]; then
	if [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
		...
		echo "      $ batchRun.bash [TARGET] [FTVAR] [ITR] [RR] [RMODE] [FTKIND] [_M_SIZE] [INPUTFILE]"
...
if [ $# -eq 8 ]; then
	...
	ftkind=$6
	inputData1=4096
	inputFile1="$rodinia/data/lud/64.dat"
elif [ $# -eq 5 ]; then
	target=$1
	ftvar=$2
	numIterations=$3
...
echo "RMODE: $RMODE"
echo "_M_SIZE: $inputData1"
echo "INPUTFILE: $inputFile1"

benchmark="lud"

#Reference output data to be compared
#refOutput="dismatch.out"
#Output data to be generated.
#tOutput="dismatch.out"
...
#	make CPU _M_SIZE=${inputData1}
#	cd ${binaryDirBase}
#	./lud_CPU -v -i ${inputFile1} 
#fi
		...
		do	
			...
			if [ -f "${binaryDir}/${targetdir}/${binary_target}" ]; then
				...
				do
					...
					exeCmd="./${binary_target} -v -i ${inputFile1}"
					if [ $printprogress -gt 0 ]; then
