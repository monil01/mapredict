...
if [ $# -ge 1 ]; then
	if [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
		...
		echo "      $ batchRun.bash [TARGET] [FTVAR] [ITR] [RR] [RMODE] [FTKIND] [SIZE] [INPUTFILE1] [INPUTFILE2]"
		...
		echo "      and SIZE is the input size for the input files, INPUTFILE1 & 2."
		echo "      (default: SIZE = 64 for INPUTFILE = temp_64 and power_64)."
		echo ""
		exit
	fi
fi
...
if [ $# -eq 9 ]; then
	...
	ftkind=$6
	inputData1=64
	inputFile1="${rodinia}/data/hotspot/temp_64"
	inputFile2="${rodinia}/data/hotspot/power_64"
elif [ $# -eq 5 ]; then
	target=$1
	ftvar=$2
	numIterations=$3
...
refOutputs=( "refhpoutput_${inputData1}" )
#Output data to be generated.
tOutputs=( "hpoutput_${inputData1}" )
...
FTVAR0Set=( "temp" "result" "power" )
FTVAR1Set=( "temp" "result" )
...
if [ ! -f ${binaryDirBase}/${refOutputs[0]} ]; then
	make CPU ROW_SIZE=${inputData1} COL_SIZE=${inputData1}
	cd ${binaryDirBase}
	./hotspot_CPU ${inputData1} ${inputData1} 100 1 ${inputFile1} ${inputFile2} ${refOutputs[0]}
fi
		...
		do	
			...
			if [ -f "${binaryDir}/${targetdir}/${binary_target}" ]; then
				...
				do
					...
					exeCmd="./${binary_target} ${inputData1} ${inputData1} 100 1 ${inputFile1} ${inputFile2} ${tOutputs[0]}"
					if [ $printprogress -gt 0 ]; then
