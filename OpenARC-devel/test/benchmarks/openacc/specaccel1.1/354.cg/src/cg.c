...
int main(int argc, char *argv[])
{
  ...
  printf("\n\n NAS Parallel Benchmarks (NPB3.3-OPENACC-C) - CG Benchmark\n\n");
  printf(" Size: %11d\n", NA);
...
#ifndef USE_STATICMALLOC
#ifdef USE_UNIFIEDMEM
  colidx=(int*)acc_create_unified(NULL, sizeof(int)*NZ);
  rowstr=(int*)acc_create_unified(NULL, sizeof(int)*(NA+1));
  a=(double*)acc_create_unified(NULL, sizeof(double)*NZ);
  x=(double*)acc_create_unified(NULL, sizeof(double)*(NA+2));
  z=(double*)acc_create_unified(NULL, sizeof(double)*(NA+2));
  p=(double*)acc_create_unified(NULL, sizeof(double)*(NA+2));
  q=(double*)acc_create_unified(NULL, sizeof(double)*(NA+2));
  r=(double*)acc_create_unified(NULL, sizeof(double)*(NA+2));
#else
  colidx=(int*)malloc(sizeof(int)*NZ);
  rowstr=(int*)malloc(sizeof(int)*(NA+1));
...
#pragma acc wait(0)
  ...
  for (it = 1; it <= 1; it++) {
    ...
    norm_temp2 = 0.0;
#ifdef USE_ASYNC
#pragma acc kernels loop independent reduction(+:norm_temp1,norm_temp2) async(0)
#else
#pragma acc kernels loop independent reduction(+:norm_temp1,norm_temp2)
#endif
    for (j = 0; j < end; j++) {
...
#pragma acc wait(0)
  ...
  print_results("CG", ClassT, NA, 0, 0,
                nit, t,
                      ...
                      double *rnorm)
{
  ...
  rho = 0.0;
#pragma acc data pcopyin(colidx[0:NZ],rowstr[0:NA+1],x[0:NA+2],a[0:NZ]), \
                 pcopyout(z[0:NA+2],p[0:NA+2],q[0:NA+2],r[0:NA+2])
{
  //---------------------------------------------------------------------
  ...
  //---------------------------------------------------------------------
#ifdef USE_ASYNC
  #pragma acc kernels loop independent reduction(+:rho) async(0)
#else
  #pragma acc kernels loop independent reduction(+:rho) 
#endif
...
#ifdef USE_ASYNC
    #pragma acc parallel loop private(tmp1, tmp2, sum) async(0)
#else
    #pragma acc parallel loop private(tmp1, tmp2, sum)
#endif
    for (j = 0; j < end; j++) {
       tmp1 = rowstr[j];
       tmp2 = rowstr[j+1];
...
#ifdef USE_ASYNC
    #pragma acc kernels loop independent reduction(+:d) async(0)
#else
    #pragma acc kernels loop independent reduction(+:d) 
#endif
    for (j = 0; j < end; j++) {
      d = d + p[j]*q[j];
    }
...
#pragma acc wait(0)
...
#ifdef USE_ASYNC
  #pragma acc parallel loop private(tmp1, tmp2, tmp3, d) async(0)
#else
  #pragma acc parallel loop private(tmp1, tmp2, tmp3, d)
#endif
  for (j = 0; j < end; j++) {
    tmp1=rowstr[j];
    tmp2=rowstr[j+1];
    d = 0.0;
    #pragma acc loop reduction(+:d) private(tmp3)
    for (k = tmp1; k < tmp2; k++) {
        tmp3=colidx[k];
        d = d + a[k]*z[tmp3];
    }
  ...
  sum = 0.0;
#ifdef USE_ASYNC
  #pragma acc kernels loop independent reduction(+:sum) private(d) async(0)
#else
  #pragma acc kernels loop independent reduction(+:sum) private(d)
#endif
  for (j = 0; j < lastcol-firstcol+1; j++) {
    d   = x[j] - r[j];
    sum = sum + d*d;
