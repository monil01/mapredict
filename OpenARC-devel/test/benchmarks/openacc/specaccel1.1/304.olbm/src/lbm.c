...
#ifdef _OPENARC_
#pragma openarc #define GRID_SIZE (SIZE_Z*SIZE_Y*SIZE_X*N_CELL_ENTRIES)
#endif
...
void LBM_allocateGrid( double** ptr, double ** org_ptr ) {
	...
	*ptr = (double *)acc_create_unified(NULL,  size);
#else
...
#ifdef USE_UNIFIEDMEM
	acc_delete_unified( *ptr-margin, 0 );
#else
	free( *ptr-margin );
#endif
	*ptr = NULL;
...
#pragma acc data present(srcGrid[0:GRID_SIZE], dstGrid[0:GRID_SIZE])
	{
#ifdef USE_ASYNC
#pragma acc parallel loop  gang worker private(ux, uy, uz, u2, rho) async(0)
#else
#pragma acc parallel loop  gang worker private(ux, uy, uz, u2, rho)
#endif
...
#pragma acc data present(srcGrid[0:GRID_SIZE])
	  {
#ifdef USE_ASYNC
#pragma acc parallel loop gang worker private(ux, uy, uz, rho, ux1, uy1, uz1, rho1, ux2, uy2, uz2, rho2, u2, px, py) async(0)
#else
#pragma acc parallel loop gang worker private(ux, uy, uz, rho, ux1, uy1, uz1, rho1, ux2, uy2, uz2, rho2, u2, px, py)
#endif
