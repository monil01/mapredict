	...
	do	
		...
		do	
			...
			if [ "$conffile" = "" ]; then
				...
				rm "Makefile.temp"
				make $target -f "$localmakefile" R_MODE=${RMODE} SPMUL_INPUTDIR="$spmulinput/" 2>&1 | tee -a ${baseDir}/$compilelog
			else
				...
				make $target R_MODE=${RMODE} SPMUL_INPUTDIR="$spmulinput/" 2>&1 | tee -a ${baseDir}/$compilelog
			fi
			cd ${binaryDir}
