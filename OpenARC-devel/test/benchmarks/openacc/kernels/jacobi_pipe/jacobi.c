...
#pragma acc data copy(b[0:SIZE_2][0:SIZE_2]), pipe(a[0:SIZE_2][0:SIZE_2])
    for (k = 0; k < ITER; k++)
    {
#ifdef _OPENARC_
#pragma openarc transform permute(j,i)
#endif
#pragma acc kernels loop pipeout(a), gang, worker
        for (i = 1; i <= SIZE; i++)
        {
            for (j = 1; j <= SIZE; j++)
            {
...
#pragma acc kernels loop pipein(a) gang worker
#pragma openarc transform permute(j,i)
        for (i = 1; i <= SIZE; i++)
        {
            for (j = 1; j <= SIZE; j++)
