...
{	
	...
	double f;
#if (MEM == NVL) && POOR
	NVL_PREFIX double *low = 0;
	NVL_PREFIX double *high = 0;
#else
	double *low, *high;
#endif
...
#if (MEM == NVL) && !POOR
		low = nvl_bare_hack(nuclide_grids + nuc*n_gridpoints*6 + (grid_ptrs[n_isotopes*idx + nuc] - 1)*6);
		high = nvl_bare_hack(nuclide_grids + nuc*n_gridpoints*6 + (grid_ptrs[n_isotopes*idx + nuc])*6);
#else
...
#if (MEM == NVL) && !POOR
		low = nvl_bare_hack(nuclide_grids + nuc*n_gridpoints*6 + (grid_ptrs[n_isotopes*idx + nuc])*6);
		high = nvl_bare_hack(nuclide_grids + nuc*n_gridpoints*6 + (grid_ptrs[n_isotopes*idx + nuc] + 1)*6);
#else
...
#if (MEM == NVL) && !POOR
	double *A_v = nvl_bare_hack(A);
#endif

	while(length > 1){
		examinationPoint = lowerLimit + (length/2);	
#if (MEM == NVL) && !POOR
		if(A_v[examinationPoint] > quarry) upperLimit = examinationPoint;
