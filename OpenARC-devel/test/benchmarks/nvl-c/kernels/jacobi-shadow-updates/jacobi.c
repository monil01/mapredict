...
# error unknown MEM setting
    ...
    printf ("(NVL mode, is_pmem=%d, ",
            pmem_is_pmem(nvl_bare_hack(b_nv), SIZE_2*SIZE_2*sizeof(float)));
# if TXS
...
# endif
	...
	#pragma nvl atomic heap(heap) default(readonly) \
	backup(k_nv[0:1]), backup_writeFirst(b_nv[1:SIZE][0:SIZE_2])
#else
...
#endif
          for (j = 1; j <= SIZE; j++)
            b_nv[i][j] = (a_v[i - 1][j] + a_v[i + 1][j] + a_v[i][j - 1] + a_v[i][j + 1]) / 4.0f;
#if TXS == 4
	  b_nv[i][SIZE_1] = 1.0;
#endif
        }
