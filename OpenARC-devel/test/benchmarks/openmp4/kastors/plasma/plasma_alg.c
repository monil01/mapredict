...
void plasma_pdgetrf_rectil_quark(PLASMA_desc A, int *IPIV)
{
	...
	// Panel is the vertical tile-column
		
    CORE_dgetrf_rectil_init();

	/// FOR EACH TILE IN THE DIAGONAL

	void* matrix = A.mat;
	int matrixSize = A.bsiz;
	int tileSize = A.mb*A.nb;

#pragma omp target data map (matrix[0:matrixSize])
    for (k = 0; k < min(A.mt, A.nt); k++)
    {
        ...
        double *dA = A(k, k);
        int *dB = IPIV(k);
		/// This is the panel (vertical, under k,k)
        PLASMA_desc pDesc = plasma_desc_submatrix(A, tempk, k*A.nb, tempm, tempkn);
		int pDesc_n=pDesc.n;
//#pragma omp target teams distribute nowait shared(pDesc, dB) 
#pragma omp task depend(inout:dA[0:tileSize]) depend(out:dB[0:pDesc_n])
        {
            int info[3];
            info[1] = 0;
            info[2] = 1;

			// Apply LU factorization to the panel (pdesc))
        ...
        for (n = k+1; n < A.nt; n++)
        {
            ...
            double *dB = A(k, k);
            int *dipiv = IPIV(k);
// #pragma omp target teams distribute nowait shared(descA, tempkm,dipiv,dB,ldak) 
#pragma omp task depend(inout:dA[0:1]) depend(in:dB[0:ldak], dipiv[0:tempkm])
            ...
            if ( m < A.mt ) {
                ...
                double *dC = A(m , n);
				int A_mb = A.mb;
//#pragma omp target teams distribute nowait shared(tempmm, tempnn,A_mb,dB,ldak) 
#pragma omp task depend(in:dA[0:tileSize], dB[0:tileSize]) depend(inout:dC[0:tileSize])
...
#pragma omp task depend(in:dA[0:A.tileSize], dB[0:A.tileSize], fake2[0:1]) depend(inout:dC[0:A.tileSize], fake1[0:A.tileSize])
                        cblas_dgemm(CblasColMajor, (CBLAS_TRANSPOSE)PlasmaNoTrans, (CBLAS_TRANSPOSE)PlasmaNoTrans,
                                tempmm, tempnn, A.nb,
                                mzone, dA, ldam,
                                dB, ldak,
