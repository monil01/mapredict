...
double run(struct user_parameters* params)
{
	...
	if( matrix_size != MAT_SIZE ) {
		params->succeed = 0;
		params->string2display = "*****ERROR: matrix size and macro MAT_SIZE should be identical";
		return 0;
	}
...
#pragma omp task firstprivate(i,j) private(ii,jj)
	...
	if(params->check) {
		...
		double (*udiff)[MAT_SIZE] = (double (*)[MAT_SIZE])udiff_;
		/// CHECK OUTPUT
...
double r8mat_rms(int nx, int ny, double *a_) {
	double (*a)[MAT_SIZE] = (double (*)[MAT_SIZE])a_;
	int i;
	int j;
	double v;
