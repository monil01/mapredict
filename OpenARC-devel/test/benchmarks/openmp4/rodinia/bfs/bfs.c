...
map(to:h_graph_visited[0:NUM_OF_NODES], h_graph_nodes_starting[0:NUM_OF_NODES], \
...
map(tofrom:h_cost[0:NUM_OF_NODES]) 
	do
	{
		//if no thread changes this value then the loop stops
		stop=false;

		#pragma omp target teams distribute parallel for private(tid,i) 
		...
		#pragma omp target teams distribute parallel for private(tid) 
  		for(tid=0; tid< NUM_OF_NODES ; tid++ )
		{
