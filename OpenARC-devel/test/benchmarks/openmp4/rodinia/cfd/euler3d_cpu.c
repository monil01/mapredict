...
int main(int argc, char* argv[])
{
		...
		start_time2 = gettime();
#pragma omp target data map(from:variables[0:NELR*NVAR]) \
        map(to:ff_variable[0:NVAR], areas[0:NELR]) \
        map(to:elements_surrounding_elements[0:NELR*NNB], normals[0:NDIM*NNB*NELR]) \
        map(alloc:fluxes[0:NELR*NVAR]) \
        map(alloc:old_variables[0:NELR*NVAR], step_factors[0:NELR])
		{
