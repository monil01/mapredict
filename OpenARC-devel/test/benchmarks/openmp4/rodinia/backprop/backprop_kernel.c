...
map(tofrom:input_weights[0:H_SIZE][0:I_SIZE]) \
map(tofrom:hidden_weights[0:O_SIZE][0:H_SIZE]) \
map(to:input_prev_weights[0:H_SIZE][0:I_SIZE]) \
map(to:input_units[0:I_SIZE]) \
map(to:hidden_prev_weights[0:O_SIZE][0:H_SIZE]) \
map(alloc:hidden_units[0:H_SIZE])
{
#ifdef DEBUG
  end_time = gettime();
  ...
  //bpnn_layerforward2(hidden_units, output_units, (float (*)[H_SIZE])hidden_weights, hid, out);
    /*** Set up thresholding unit ***/
  //hidden_units[0] = 1.0F;
  #pragma omp target update to(hidden_units[0:H_SIZE])
  /*** For each unit in second layer ***/
  for (j = 1; j <= out; j++) {
  ...
  //hidden_units[0] = 1.0F;
    ...
    map(to:output_delta[0:O_SIZE]) private(new_dw)
  for (k = 0; k <= hid; k++) {
  	for (j = 1; j <= out; j++) {
  ...
  //input_units[0] = 1.0F;
  	//input_units[0] = 1.0F;
  	#pragma omp target teams distribute parallel for \
  	map(to:hidden_delta[0:H_SIZE]) private(new_dw)
  for (k = 0; k <= in; k++) {
  	for (j = 1; j <= hid; j++) {
