...
#pragma openarc #define _SIZE_I_ (rows * cols)

#if _BSIZE_ == 8
#pragma openarc #define _BSIZE_ 8
#elif _BSIZE_ == 16
#pragma openarc #define _BSIZE_ 16
#endif
#endif

#pragma aspen declare param(rows:_ROWS_,cols:rows)
int rows, cols, size_I, size_R, niter = 10;
#pragma aspen declare param(r1:0, r2:127, c1:0, c2:127)
int r1, r2, c1, c2;
float lambda;
int nthreads;
...
#endif    
*/
		mainComp1(J, iN, iS, jE, jW, dN, dS, dW, dE, c);
		mainComp2(J, iN, iS, jE, jW, dN, dS, dW, dE, c);
...
void mainComp1(float J[_SIZE_I_], int iN[_ROWS_], int iS[_ROWS_],
int jE[_COLS_], int jW[_COLS_], float dN[_SIZE_I_], float dS[_SIZE_I_],
float dW[_SIZE_I_], float dE[_SIZE_I_], float c[_SIZE_I_])
{
	int iter, k;
...
//#pragma aspen declare param(aspen_param_prob1:1/cols)
//#pragma aspen control probability(aspen_param_prob1, aspen_param_prob1)
#pragma aspen control probability(0, 0)
				if( i == 0 ) {
                dN[i*cols + j] = J[j] - Jc;
                dS[i*cols + j] = J[cols + j] - Jc;
				} else if( i == (rows-1) ) {
                dN[i*cols + j] = J[(i-1) * cols + j] - Jc;
                dS[i*cols + j] = J[i*cols + j] - Jc;
				} else {
                dN[i*cols + j] = J[(i-1) * cols + j] - Jc;
                dS[i*cols + j] = J[(i+1) * cols + j] - Jc;
				}
//#pragma aspen control probability(aspen_param_prob1, aspen_param_prob1)
#pragma aspen control probability(0, 0)
				if( j == 0 ) {	
                dW[i*cols + j] = J[i * cols] - Jc;
                dE[i*cols + j] = J[i * cols + 1] - Jc;
				} else if( j == (cols-1) ) {
                dW[i*cols + j] = J[i * cols + (j-1)] - Jc;
                dE[i*cols + j] = J[i*cols + j] - Jc;
				} else {
                dW[i*cols + j] = J[i * cols + (j-1)] - Jc;
                dE[i*cols + j] = J[i * cols + (j+1)] - Jc;
				}
			
                G2 = (dN[i*cols + j]*dN[i*cols + j] + dS[i*cols + j]*dS[i*cols + j] 
                    + dW[i*cols + j]*dW[i*cols + j] + dE[i*cols + j]*dE[i*cols + j]) / (Jc*Jc);

   		        L = (dN[i*cols + j] + dS[i*cols + j] + dW[i*cols + j] + dE[i*cols + j]) / Jc;
                ...
                c[i*cols + j] = 1.0F / (1.0F+den) ;
                
                // saturate diffusion coefficent
#pragma aspen  control execute stores((1*aspen_param_sizeof_float):to(c):traits(stride(1))) loads((1*aspen_param_sizeof_float):from(c):traits(stride(1)))
		...
		for (i = 0; i < rows; i++) {
            ...
            for (j = 0; j < cols; j++) {        
                ...
                // diffusion coefficent
					cN = c[i*cols + j];
#pragma aspen  control execute loads((1*aspen_param_sizeof_float):from(c):traits(stride(1)    ))
					if( i == (rows-1) ) {
						cS = c[i*cols + j];
					} else {
						cS = c[(i+1) * cols + j];
...
#pragma aspen  control execute loads((1*aspen_param_sizeof_float):from(c):traits(stride(1)    ))
                ...
                // divergence (equ 58)
                D = cN * dN[i*cols + j] + cS * dS[i*cols + j] + cW * dW[i*cols + j] + cE * dE[i*cols + j];
                
                // image update (equ 61)
                J[i*cols + j] = J[i*cols + j] + 0.25F*lambda*D;
            }
	     }
		 #pragma acc update host(J[0:_SIZE_I_])
...
void mainComp2(float J[_SIZE_I_], int iN[_ROWS_], int iS[_ROWS_],
int jE[_COLS_], int jW[_COLS_], float dN[_SIZE_I_], float dS[_SIZE_I_],
float dW[_SIZE_I_], float dE[_SIZE_I_], float c[_SIZE_I_])
{
	int iter, k;
...
#endif    
*/
        #pragma omp parallel for shared(J, dN, dS, dW, dE, c, rows, cols) private(i, j, k, Jc, G2, L, num, den, qsqr)
		for (i = 0 ; i < rows ; i++) {
            for (j = 0; j < cols; j++) { 
