...
#pragma acc kernels loop gang, worker
        for( i=0; i<SIZE2; i++ ) { 
            y[i] = 0.0f;
#pragma aspen declare param(aspen_param_rowptrcnt:10)
#pragma aspen control loop(aspen_param_rowptrcnt)
...
#pragma acc kernels loop gang, worker
		for( i=0; i<SIZE2; i++ ) {
			float tmp = y[i];
			x[i] = tmp;
#pragma aspen declare param(aspen_param_nonzeroprob:0.99)
#pragma aspen control probability(aspen_param_nonzeroprob)
			if( tmp != 0.0f ) {
				exp0 = (int)(log10f(fabsf(tmp)));
#pragma aspen declare param(aspen_param_normalizecnt:1)
#pragma aspen control probability(0.5,0.5)
				if( exp0 >= 0 ) {
#pragma aspen control loop(aspen_param_normalizecnt)
					for( j=1; j<=(1+exp0); j++ ) {
						x[i] = x[i]/10.0f;
					}
				} else if( exp0 <= -1 ) {
