...
#if TESTMODE == 1
#ifdef TEST_ASYNC
#pragma acc kernels loop independent gang(GANGS) worker(WORKERS) reduction(+:C[0:MSIZE], D) async
#else
...
#else
#ifdef TEST_ASYNC
#pragma acc kernels loop independent gang(GANGS) worker(WORKERS) reduction(+:C[0:MSIZE]) async
#else
...
#ifdef TEST_ASYNC
#pragma acc kernels loop independent gang(GANGS) worker(WORKERS) reduction(+:D) async
#else
